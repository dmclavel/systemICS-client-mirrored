	...
	handleSearch = query => {
		if (query.length !== 0) {
			if (this.state.activeItem === 'Student') {
				this.setState({
					dummyStudents: this.state.dummyStudents.filter(student => {
						if (
							...
							student.email_add.toLowerCase().includes(query.toLowerCase())
						) {
							return true;
						} else {
							return false;
				...
				this.setState({
					dummyFaculty: this.state.dummyFaculty.filter(faculty => {
						if (
							faculty.name.toLowerCase().includes(query.toLowerCase()) ||
							faculty.email_add.toLowerCase().includes(query.toLowerCase())
						) {
							return true;
						} else {
							return false;
			...
			<Grid>
				...
				<Grid.Row>
					{this.state.activeItem === 'Student' ? (
						...
						<FacultyTable
							data={this.state.dummyFaculty}
							fetchData={this.fetchFaculty}
						/>
					)}
				</Grid.Row>
				<Grid.Row>
