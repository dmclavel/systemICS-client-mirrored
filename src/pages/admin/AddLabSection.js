...
import { Input, Button, Modal, Form, Grid, Segment, Header, Divider, Container, Checkbox, Popup, Label } from 'semantic-ui-react';
...
class AddCourseLab extends Component {
	...
	handleSubmit = () => {
		...
		const{courses, course_offering_id, course_id, course_name, emp_no, acad_year, semester, no_of_students, time_start, time_end, room, day, section, unit, max_capacity, course_title, status, description, course} = this.state

		const socket = socketIOClient(this.state.address);

		const data = {email: 'pvgrubat@up.edu.ph', acad_year:acad_year, semester:semester,
					  time_start:time_start, time_end:time_end, room:room, no_of_students:no_of_students,
					  unit:unit, day:days, section:section, max_capacity:max_capacity,
					  course_id:id, emp_no:null, status:"Pending"};
		this.setState({error:''});
		if(section == '' ||
			 room == '' ||
			 time_start == '' ||
  ...
  render() {
		const{error, M, T, W, Th, F, courses, course_offering_id, course_id,
			  course_name, emp_no, acad_year, semester, no_of_students, time_start, time_end, room,
			  day, section, unit, max_capacity, course_title, status, description, course} = this.state
            	...
            	<Grid>
            		...
            		<Header as="h2">Add New Laboratory </Header>
								...
								<Grid.Row>
									<Form >
										<Form.Group>
											<Form.Input label="Section" placeholder="Course section" name="section" value={section} onChange={this.handleChange}/>
		            			...
		            			<Form.Input type="time" label="Time start" placeholder="Time start" name="time_start" value={time_start} onChange={this.handleChange}/>
										...
										<Form.Group>
											...
											<Form.Input type="number" label="Acad Year" name="acad_year" placeholder="Acad Year" value={acad_year} onChange={this.handleChange}/>
											<Form.Input type="number" label="Semester" name="semester" placeholder="Semester" value={semester} onChange={this.handleChange}/>
		            			...
		            			<Form.Field>
										...
										<Button content="Submit" floated="left" onClick={this.handleSubmit}/ >
